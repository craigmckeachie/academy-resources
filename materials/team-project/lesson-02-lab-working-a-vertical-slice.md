---
title: "Lesson 2 Lab — Working a vertical slice"
---

# Lesson 2 Lab — Working a Vertical Slice
...
1. At standup, confirm **who the database schema owner is** this sprint — the one person allowed
    to change the database. If it isn't you, you do not run `Add-Migration` — not once, not to
    check something.
    [Guide §2](lesson-02-guide-depth-in-a-shared-codebase.md#2-migrations-in-a-team-the-schema-owner)
    has the half-day mess this prevents.
2. Read your issue and write down **which layers it touches**, in this order:
...
3. If your ticket has a **dependency** on a teammate's — it says so on the issue — find out where
    theirs stands before you plan your day.
4. **When the schema owner's migration merges**, stop what you're doing and take it:
...
6. Work outside-in or inside-out, but **finish one layer before you start the next** — an endpoint
    you've verified in Insomnia is a fixed point you can build the page against. Half of each is
    the slowest way to do this.
7. **If you're adding a parameter to an endpoint that already has callers** —
...
8. **If you're adding a navigation property to a foreign key that already exists**, that is not a
    schema change and `Add-Migration` isn't yours to run —
    [guide §2](lesson-02-guide-depth-in-a-shared-codebase.md#2-migrations-in-a-team-the-schema-owner)
    on why an **empty** migration is the correct outcome there.
9. **Push and open the pull request early** — while it's still small, even if it's a draft. A
...
| **changes only the front end** | the feature, then **reload the page** | — | — |

10. Work your row of that table.
11. Then the check that catches the failure a diff can't show you: **change the control and watch
    the Network tab.** A parameter spelled one way in the controller and another way in the API
    module produces no error anywhere — the value just never arrives. Read the request URL that
    actually goes out; don't infer it from the code.
12. And the quiet one from
    [guide §3](lesson-02-guide-depth-in-a-shared-codebase.md#3-extending-an-endpoint-without-breaking-it):
    **look at every column the page was already showing.** A
    dropped `Include` doesn't throw. It empties a column.
13. Run through your issue's acceptance criteria one at a time and tick them off. The charter's
...
14. **Before you open the merge editor, find out what the other change was for.** The issue number
    is in the branch name and on the pull request. Read the issue.
15. Resolve for **both intents** — not for whichever side looks tidier, and not by accepting one
    side because the result compiles.
16. Confirm no markers survived — search the project for `<<<<<<<`.
17. **Run the app and exercise both features.** Yours *and* theirs, together, in one session. This
    is the step that catches a wrong resolution, and nothing else does.
18. Say in the pull request **what you merged and why**. Your reviewer is about to approve a
...
!!! warning "A clean resolution is not a correct resolution"
    ...
    **compile and render a working page** — with one person's feature silently doing nothing.
    ...
    the four options and which three of them lie to you. The only proof is step 17.
...
19. Fill in all three sections. On a vertical slice, "How I verified it" is where the work shows:
...
20. **Review a teammate's**, and rotate — not the same person as last time. Run it, then work
...
21. Leave at least one line comment, then submit **Approve** or **Request changes** with a summary
...
22. Once yours is approved: squash-merge, then
