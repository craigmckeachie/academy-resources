# Lesson 1 Lab — CategoriesController
...
- **Filter by name** — [Reinforce] — accept an optional `?name=` query string on
  ...
  shape as the `?status=` filter you'll write on Orders in Lesson 3.)
- **404 on a bad id** — [Reinforce] — make `GetById` return `NotFound()` when the
  id doesn't exist, ahead of Lesson 2 formally covering status codes.
