# Lesson 3 Guide — Detail Pages, Modals, the Nested Child Form, and Sign In
...
- `.detail-header` is a small custom class in `styles.css` (`display:flex;
  ...
  form page.
- **Status** reuses the contextual **badge** from Lesson 2, matching the order's
  status color.
...
Lesson 2's, with two twists: it's **scoped to a parent** (Cancel returns to the order
